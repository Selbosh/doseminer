...
author: David Selby, Belay Birlie and Katherine Dempsey
output:
  github_document:
    df_print: kable
---
...
**Redesign analysis to extract and strip text sequentially. Based on the assumption that, e.g. frequency and dose number are disjoint**
...
and so on. Clearly, the functions to convert extracted strings into numeric
dosages are not ready, yet. But the basic principle should be fairly clear.
...
results[is.na(results)] <- '' # for presentation purposes
results
```
